...
impl MetricsCollector {
    ...
    /// metrics from the token store and updates the OpenTelemetry gauges.
    pub fn start_collection<T: TokenStore + 'static>(
        &self,
        token_store: Arc<T>,
...
pub fn init_metrics_collection<T: TokenStore + 'static>(
    token_store: Arc<T>,
    ...
    collector.start_collection(token_store, collection_interval);
...
mod tests {
    use super::*;
    use crate::test_utils::MockTokenStore;
    ...
    async fn test_init_metrics_collection() {
        let mock_store = Arc::new(MockTokenStore::new().with_token_count(10));
        let interval = Duration::from_millis(100);

        let collector = init_metrics_collection(mock_store, interval);
