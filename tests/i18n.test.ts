import { I18n, translations, type LanguageCode, type Translations } from '../server/src/includes/i18n';
...
describe('I18n Translation Completeness', () => {
  ...
  describe('Translation Key Categories', () => {
    const testCategoryExists = (category: string, expectedMinCount: number) => {
      ...
      const categoryKeys = enKeys.filter(key => key.startsWith(`${category}.`));
  ...
  describe('Translation Quality', () => {
    ...
    test('should have consistent punctuation patterns within languages', () => {
      ...
      if (enLabels.length > 0) {
        const enLabelsWithColons = enLabels.filter(label => label.endsWith(':'));
  ...
  describe('Critical Translation Keys', () => {
    ...
    test('should have translations for critical UI elements', () => {
      ...
      const existingCriticalKeys = possibleCriticalKeys.filter(key => actualKeys.includes(key));
  ...
  describe('Translation Completeness Report', () => {
    test('should generate a completeness report', () => {
      ...
      console.log('Translation Completeness Report:', JSON.stringify(report, null, 2));
