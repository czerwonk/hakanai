...
pub async fn send<T: Factory>(factory: T, args: SendArgs) -> Result<()> {
    ...
    let restrictions = args.get_restrictions();
...
mod tests {
    ...
    async fn test_send_zero_ttl() -> Result<()> {
        ...
        assert!(
            result.is_err(),
            "Expected error for zero TTL, got: {:?}",
            result
        );
        assert!(
            result
    ...
    async fn test_send_empty_secret_from_file() -> Result<()> {
        ...
        assert!(
            result.is_err(),
            "Expected error for empty secret, got: {:?}",
            result
        );
        assert!(
            result
    ...
    async fn test_send_client_error() -> Result<()> {
        ...
        assert!(
            result.is_err(),
            "Expected error for network failure, got: {:?}",
            result
        );
    ...
    async fn test_send_with_all_restrictions() -> Result<()> {
        ...
        fs::write(&file_path, b"restricted secret content")?;
